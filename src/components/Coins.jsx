...
const Coins = () => {
  ...
  useEffect(() => {
    ...
    const fetchCoins = async () => {
      try {
        const res = await fetch(`${server}/coins/markets?vs_currency=${currency}&page=${page}`);
        ...
        (<Loader />) : (
          ...
          <>
            ...
            <HStack w={'full'} overflowX={'auto'} p={'8'}>
              {
                btns.map((item, index) => (
                  <Button key={item} bgColor={"blackAlpha.900"} color={'white'} onClick={() => changePage(index + 1)}>
                    {index + 1}
                  </Button>
                ))}
